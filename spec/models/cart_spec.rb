...
describe Cart do
  it "can add one need to cart if within max amount" do
    need = create(:need)
    need_type = create(:need_type)
    cart = Cart.new({}).add_need(need, need_type.max)

    expect(cart).to eq("Successfully added to cart!")
  end

  it "cannot add more than the max allowable quantity" do
     needs = 6.times.map  { create(:need) }
     need_type = create(:need_type)

     cart = Cart.new({}).add_need(needs, need_type.max)
     expect(cart).to eq("Successfully added to cart!")
  end
end
